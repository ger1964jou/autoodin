...
const uint32_t timestep = 150000;                   // time in ms between scans                                                          // re set to 1300000 after debugging
...
void setup() {
    ...
    uint8_t i;
    for (i = 0; i < 8; ++i) {                                                                      // no different from i++ in this context
        pinMode(i , OUTPUT);                 //Configure all channel control pins as output        // not using pins 0, 1, & 2 so why set them?
...
void calibrate() {
    ...
    for (int k = 0; k < channels; k = k + 1) {                                                     // should keep consistent with ++ notation // re-set |k < channels| after testing
        Serial.print("\nChannel ");
     ...
     //       Serial.println(value);
            Serial.println();
            if (abs(value - 100) < abs(best_value - 100)) {
              best_value = value;
              best_pwm = mid;
            }
            if (value > 100) {
        ...
        Serial.print("Best Value: ");
        Serial.println(best_value);
        data.adc[k + channelstart] = best_pwm;
        pwmlevel[k + channelstart] = best_pwm;
        */
        
        
        data.adc[k + channelstart] = best_pwm; // was mid
        pwmlevel[k + channelstart] = best_pwm; // was mid
...
double average(int8_t pin) {                            // averages the analog input 30 times to return a value
    ...
    a = a/30;
    Serial.println(a);
    Serial.print("high of average ");
    Serial.println(high);
    Serial.print("low of average ");
    Serial.println(low);
    return a;
}
...
void channel_select(uint8_t channel) {
    ...
    uint8_t i;
    //Serial.print("\t");
    for (i = 0; i < addr_bits; ++i) {                                                                        // would be easier to comprehend if unrolled
