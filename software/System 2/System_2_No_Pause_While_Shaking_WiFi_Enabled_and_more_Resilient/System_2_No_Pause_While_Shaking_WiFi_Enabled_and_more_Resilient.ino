/*
  ...
  Last updated Feb 23, 2015 (Jonathan Forbes)
  Updated to make the WiFi routine more resilient to occasional failure
  Based on System 1 software written by Bob Petersen and David Beck
*/
...
#define WLAN_SSID       "Stahl_Lab_Netgear"        // Wireless Network Identifier (cannot be longer than 32 characters!)   The other network in the lab: "Stahl_Lab_Linksys_1"
#define WLAN_PASS       "sulfate2010"              // Wireless Network Password
#define WLAN_SECURITY   WLAN_SEC_WPA2              // Wireless Network Security Algorithm

#define ADAFRUIT_CC3000_IRQ   2                    // Wifi interrupt pin on the Arduino
#define ADAFRUIT_CC3000_VBAT  5                    // Power enable pin for the wifi chip
#define ADAFRUIT_CC3000_CS    49                   // Chip-select pin for the wifi since it communicates with the Arduino via SPI
...
Adafruit_CC3000 cc3000 = Adafruit_CC3000(ADAFRUIT_CC3000_CS, ADAFRUIT_CC3000_IRQ, ADAFRUIT_CC3000_VBAT, SPI_CLOCK_DIVIDER); // Create the CC3000 Wifi object, using the pins specified in CC3000.h
uint32_t ip = cc3000.IP2U32(128, 208, 236, 38);    // IP Address of the server to send the UDP data packet to
uint16_t port = 8577;                              // Port on the server to address the packet to
Adafruit_CC3000_Client udp;                        // Create the CC3000 client UDP connection
...
const int adc_samples = 5;                     // number of repeated adc samples per each scan of each test tube, there can be an occasional spike in reading, this evens it out, but too much slows down the sampling
...
void loop() {
  ...
  int wifi_send_tries = 2; // the number of times to try to send the data to the server
  while (wifi_send_tries > 0) // while there are tries left, try to send the data to the server
  {
    if (send_data_to_server()) ////////////////////////////////////////////////////////////////////////////////////////WIFIWIFIWIFIWIFI
    {
      break; // end the loop, sending the data to the server was successful!
    }
    else
    {
      wifi_send_tries--; // used up a try
    }
  }
...
// Sends a UDP Packet embedded with the data structure to the Server for inclusion in FileMakerPro
boolean send_data_to_server()
{
  ...
  int begin_tries = 3;
  while (!cc3000.begin() && begin_tries > 0) // try until no more tries are left or the cc3000 is initialized.
  {
    delay(500); // wait half a second before trying again
    begin_tries--; // one try, used up
  }
  if (begin_tries == 0) // ran out of tries
  {
  ...
  if (!cc3000.connectToAP(WLAN_SSID, WLAN_PASS, WLAN_SECURITY, 3)) // try to connect only 3 times (any more takes FOREVER!)
  {
    Serial.println("Failed to connect to wifi on all 3 tries!");
    goto Wifi_end;
  } 
  else
  {
  ...
  if (connection)
  {
    ...
    lcdPrint("Requesting DHCP.....","","","");
    Serial.println("Request DHCP");
    unsigned long timeout;
    timeout = millis();
    while (!cc3000.checkDHCP())
    {
      if (millis() - timeout == 15000) // 15 second timeout for DHCP request
      {
        goto Wifi_end;
      }
      delay(100);
    ...
    /* Display the IP address DNS, Gateway, etc. */ /* // Not really necessary
    while (! displayConnectionDetails())
	{
      delay(1000);
    }*/

	lcdPrint("Preparing for UDP...","","","");
    Serial.println("Preparing for UDP connection");
	
    udp = cc3000.connectUDP(ip, port); // connect to UDP, prepare the packet connection for transmission to the server
    lcdPrint("Connecting to UDP...","","","");
    Serial.println("Waiting for UDP connection");
    timeout = millis();
    while(!udp.connected()) // wait until the UDP connection is made
    {
      if (millis() - timeout == 15000) // 15 second timeout for UDP connect
      {
        goto Wifi_end;
      }
      delay(20);
    ...
    lcdPrint("Sending UDP Packet..","","","");
    Serial.println("Sending UDP Packet");
    if (!udp.write((byte *)&data, sizeof(struct data))) // Send the packet to the server
    {
  ...
  Wifi_end: // label for various parts of the wifi routine to go-to in case of failure
  /* You need to make sure to clean up after yourself or the CC3000 can freak out */
  /* the next time you try to connect ... */
  lcdPrint("Closing connection..","","","");
  Serial.println("\n\nClosing the connection");
  cc3000.disconnect(); // disconnect from the server
  delay(200); // wait a moment for all processes to complete on the wifi chip
  //cc3000.stop(); // Don't use this! It only works once, then the Wifi chip won't come back on again...
  return success;
}
...
void raise_platform()
{
  ...
  while(actuator_position < 1000 && (millis() - motor_run_time) < 19000)  // run motor until either the platform is raised, or 19 seconds has elapsed
  {
    actuator_position = analogRead(A2); // read the actuator position
...
void lower_platform()
{
  ...
  actuator_position = analogRead(A2); // read the actuator position
  motor_run_time = millis(); // save the time when the motor started extending
  while(actuator_position > 50 && (millis() - motor_run_time) < 19000)  // run motor until either the platform is lowered, or 19 seconds has elapsed
  {
    actuator_position = analogRead(A2); // read the actuator position
    actuator_current = analogRead(A3); // read the current going to the motor
    if (actuator_current > 500) // if the current is too high (500 is just arbitrary, figure out what a better number is)
...
// Prints to the Serial console details pertaining to the connection made by the CC3000 Wifi chip
