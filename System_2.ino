...
byte mac[] = { 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED }; // MAC address for device
IPAddress self_ip(172, 28, 236, 74);                 // IP address for device
IPAddress dns(128, 95, 112, 1);                      // DNS address
IPAddress gateway(172, 28, 236, 100);                // Gateway address
IPAddress subnet(255, 255, 255, 0);                  // Subnet
IPAddress data_server_ip(172, 28, 236, 69);          // IP of Remote Server that will receive the data
const uint16_t data_server_port = 8577;              // Port used both to send the data from on this device, and used by the Remote Server to listen to.
EthernetUDP udp;                                     // UDP instance for sending
...
void setup()
{
  ...
  Serial.println("setup: configuring static ethernet");
  Ethernet.begin(mac, self_ip, dns, gateway, subnet); // No error return
  Serial.print("setup: ethernet configuration complete, IP address is: ");
  Serial.println(Ethernet.localIP());
  if (udp.begin(data_server_port) == 0) // udp.begin failed
  {
...
void loop() {
  ...
  delay(1000); // give the system a some time to ensure all SPI bus operations have completed, and that error messages can be read
...
void sendpacket()
{
  ...
  if (udp.beginPacket(data_server_ip, data_server_port) == 0) // beginPacket failed
  {
  ...
  else // beginPacket succeeded
  {
    udp.write((byte *)&data, PACKET_SIZE); // No error return
    if (udp.endPacket() == 0) // endPacket failed
    {
